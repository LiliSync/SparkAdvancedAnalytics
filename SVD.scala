...
val zipMatrix10 = termDocMatrix.zip(clusterId10)

val filter0Zip10=  zipMatrix10 filter {_._2._1==0}

//Abschneiden und cache

val cutted010= filter0Zip10.map {
...
val mat = new RowMatrix(cutted010)
val svd = mat.computeSVD(kSVD, computeU=true)

    println("Singular values: " + svd.s)
    val topConceptTerms = RunLSA.topTermsInTopConcepts(svd, 1, 3, termIds)
    val topConceptDocs = RunLSA.topDocsInTopConcepts(svd, 1, 10, docIds)
    for ((terms, docs) <- topConceptTerms.zip(topConceptDocs)) {
      println("Concept terms: " + terms.map(_._1).mkString(", "))
      println("Concept docs: " + docs.map(_._1).mkString(", "))
      println()
